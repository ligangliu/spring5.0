...
class ConfigurationClassEnhancer {
	...
	private Enhancer newEnhancer(Class<?> configSuperClass, @Nullable ClassLoader classLoader) {
		 ...
		 * 这里因为实现了BeanFactoryAware接口，所以会为其注入BeanFactory。
		 * 那么该代理类就可以拿到BeanFactory。
		 *
		 * 然后在配置了@Configuration的AppConfig中 @Bean的时候，我们怎么返回一个可用的对象呢？
		 * 就是通过使用factoryBean.getBean();第一次使用的时候就可以去注入，下次直接从缓存中拿。
		 * 就能解决@Bean{在这里又调用另一个@Bean的方法会产生多列的方式}
		 */
	...
	private static class BeanMethodInterceptor implements MethodInterceptor, ConditionalCallback {
		...
		public Object intercept(Object enhancedConfigInstance, Method beanMethod, Object[] beanMethodArgs,
			...
			/**
			 * 解决AppConfig中的@Bean中调用另一个@Bean方法
			 * intercept(Object o, Method method, Object[] objects, MethodProxy methodProxy)
			 * 当method == methodProxy，说明是第一次调用
			 * @Bean
			 * pubic Person person1() {
			 * 		return new Person1();
			 * }
			 * //当执行person2()中的person1()的时候，应该method=person1 而methodProxy=person2
			 * //所以此时应该放回false
			 * @Bean
