...
public class AnnotatedBeanDefinitionReader {
	 ...
	 * registry = spring的整个环境
	 * 就是一个bd(beanDefinition)的注册器
	 */
	...
	<T> void doRegisterBean(Class<T> annotatedClass, @Nullable Supplier<T> instanceSupplier, @Nullable String name,
		...
		/**
		 * 处理类当中的通用注解
		 * 如：lazy dependOn等注解信息
		 */
		AnnotationConfigUtils.processCommonDefinitionAnnotations(abd);
		/**
		 * 处理registger()方法中的参数额外传递的参数。(因为除了在类的注解上加入@Primary等注解，
		 * 还可以通过方法传递)
		 */
		if (qualifiers != null) {
		...
		/**
		 * 也就是将BeanDefinitionHolder(可以理解为一个Spring Bean的信息)放入一个容器中。
		 * 最后会注册到BeanFactory中Map容器中<beanName,beanDefinition>
		 */
